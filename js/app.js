...
function updateStatusText(isOwner, checking) {
  ...
  if(checking) {
    ...
    if(isOwner) {
      statusText.innerText = `You own ${editions.length} ${COLLECTION_NAME} so welcome to the Yuh club™!!   `;
    } else {
      statusText.innerText = `You don't own any ${COLLECTION_NAME} ew cringe🤮 JK`;
    }
  }
